...
class Policy(nn.Module):
    ...
    def forward_step(self, camera_obs, proprio_obs, lstm_state):
        ...
        # Flatten vis_encoding to be 2D
        vis_encoding = torch.flatten(vis_encoding, start_dim=1)  # Flatten to (batch_size, features)
        
        # Ensure proprio_obs is 2D (this might already be the case)
        proprio_obs = proprio_obs.view(proprio_obs.size(0), -1)  # Ensure it's (batch_size, proprio_dim)
        
        # Now concatenate them along the last dimension
        low_dim_input = torch.cat((vis_encoding, proprio_obs), dim=-1)  # Both should be 2D tensors now
        
        # Apply dropout and LSTM
        low_dim_input = self.dropout(low_dim_input)
        lstm_out, (h, c) = self.lstm(low_dim_input.unsqueeze(0), lstm_state)
        lstm_state = (h, c)
